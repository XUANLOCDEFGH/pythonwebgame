...
def row_to_list(board,y,x,dy,dx,yf,xf):   #trả về list của y,x từ yf,xf
...
def score_of_row(board,cordi,dy,dx,cordf,col):   #trả về một list với mỗi phần tử đại diện cho số điểm của 5 khối
...
def score_of_col(board,col):  #tính toán điểm số mỗi hướng của column dùng cho is_win;

    f = len(board)              #scores của 4 hướng đi
...
def score_of_col_one(board,col,y,x):    #trả lại điểm số của column trong y,x theo 4 hướng, key: điểm số khối đơn vị đó -> chỉ ktra 5 khối thay vì toàn bộ
...
def possible_moves(board):          #khởi tạo danh sách tọa độ có thể có tại danh giới các nơi đã đánh phạm vi 3 đơn vị
...
def TF34score(score3,score4):   #trả lại trường hợp chắc chắn có thể thắng(4 ô liên tiếp)
...
def stupid_score(board,col,anticol,y,x):    #cố gắng di chuyển y,x trả về điểm số tượng trưng lợi thế 
...
def click(x,y):
    ...
    if board[y][x] == ' ':
        ...
        if game_res in ["Computer Win", "You Win", "Draw"]:
            print (game_res)
            win = True
            return
...
def initialize(size):
    ...
    screen.setup(width=795, height=795)
    screen.setworldcoordinates(-1,size,size,-1)
    screen.bgcolor("lightgrey")
    screen.tracer(400)
    screen.title("CARO- Minmax Algorithm by Group 2")
    
    #Title
    style = ('Courier', 20, 'bold')
    turtle.color('red')
    turtle.write('     CARO- MINMAX ALGORITHM BY GROUP 2', font=style)

    #button Exit
    screen = screen.getcanvas()
    button = Button(screen.master, text="Exit", command=Exit, width=5, height=2, bg="Red",fg="White", relief="solid")
    button.pack()
    button.place(x=740, y=745)  # place the button anywhere on the screen
    #button Reset
    button = Button(screen.master, text="Reset", command=Reset, width=5, height=2, bg="Red", fg="White", relief="solid")
    button.pack()
    button.place(x=5, y=55)
    #button Return
    button = Button(screen.master, text="Return", command=Reset, width=5, height=2, bg="Red", fg="White", relief="solid")
    button.pack()
    button.place(x=5, y=105) 
    #button Start
    button = Button(screen.master, text="Start", command=Reset, width=5, height=2, bg="Red", fg="White", relief="solid")
    button.pack()
